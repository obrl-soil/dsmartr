...
#' @keywords internal
#' @param input A vector of integers n model runs long, where the integers refer to soil classes.
#' @param n_classes Integer; the total number of soil classes available to predict.
#' @return An atomic vector of prediction counts, or a vector of NA values of the same length.
#' e.g. 0 5 8 0 == soil 1 was not predicted, soil 2 was predicted 5 times, soil 3 was predicted 8
#' times, soil 4 was not predicted.
#' @note This is a helper function and not widely applicable. It is expected to be used in concert
#' with \code{\link[raster:calc]{calc}}, applied to a stack of model realisations produced by
#' \code{\link[dsmartr:iterate]{iterate}}.
#' @examples \dontrun{
#' # 100 model runs, from a cell in the middle of the heronvale demo dataset
#' cell_19291 <- c(29, 7, 7, 7, 12, 9, 29, 29, 29, 3, 16, 15, 9, 21, 24, 2, 9, 15, 34, 9, 24, 34,
#' 24, 34, 21, 24, 15, 15, 9, 21, 21, 7, 7, 34, 34, 15, 15, 21, 7, 30, 32, 21, 29, 21, 24, 29, 31,
#' 12, 24, 12, 21, 15, 15, 29, 12, 7, 21, 12, 7, 34, 9, 32, 7, 7, 32, 32, 12, 34, 29, 7, 3, 15, 15,
#' 29, 7, 3, 34, 9, 9, 33, 32, 21, 34, 29, 33, 7, 7, 31, 7, 7, 24, 21, 15, 7, 24, 29, 21, 31, 29,
#' 15, 7)
#' count_predictions(cell_19291)
#' }
#'
count_predictions <- function(input = NULL, n_classes = NULL) {
  # NB is.na(sum((input))) is faster but does not allow for cases where some runs don't make a
  # prediction on a pixel and others do. This can happen with covariates of varying extent (e.g.
  # along a coastline), but only in cases where some special options in C50::C50Control have been
  # used, e.g. winnow = TRUE). Bear in mind that in these cases, probabilities are still normalised
...
#'
#' Converts the number of times a soil class was predicted to a probability by normalising against
#' the total number of model runs.
#' @keywords internal
#' @param input A vector of integers n soil classes long, where the integers refer to counts of soil
#' classes.
#' @param n_iterations Integer; the total number of model runs.
#' @return An atomic vector of probabilities, or a vector of NA values of the same length.
#' e.g. for 20 model runs, 0 5 8 0 becomes 0 0.25 0.4 0.
#' @note This is a helper function and not widely applicable. It is expected to be used in concert
#' with \code{\link[raster:calc]{calc}}, applied to a stack of tabulated class counts produced
#' by \code{\link[dsmartr:collate]{collate}}.
#' @examples \dontrun{
#' # 100 model runs, from a cell in the middle of the heronvale demo dataset
#' cell_19291 <- c(29, 7, 7, 7, 12, 9, 29, 29, 29, 3, 16, 15, 9, 21, 24, 2, 9, 15, 34, 9, 24, 34,
#' 24, 34, 21, 24, 15, 15, 9, 21, 21, 7, 7, 34, 34, 15, 15, 21, 7, 30, 32, 21, 29, 21, 24, 29, 31,
...
#' @keywords internal
#' @param input A vector of integers n model runs long, where the integers refer to counts of soil
#' classes.
#' @param n_classes Integer; the total number of soil classes available to predict.
#' @return An atomic vector of probabilities, or a vector of NA values of the same length. Ties are
#' shuffled at random.
#' e.g. for 20 model runs, e.g. 0 5 8 0 becomes either 3 2 1 4 or 3 2 4 1. These values should
#' correspond to a lookup table ID column, so they can be linked to a soil class name.
#' @note This is a helper function and not widely applicable. It is expected to be used in concert
#' with \code{\link[raster:calc]{calc}}, applied to a stack of tabulated model realisations produced
#' by \code{\link[dsmartr:collate]{collate}}.
#' @examples \dontrun{
#' # 100 model runs, from a cell in the middle of the heronvale demo dataset
#' cell_19291 <- c(29, 7, 7, 7, 12, 9, 29, 29, 29, 3, 16, 15, 9, 21, 24, 2, 9, 15, 34, 9, 24, 34,
...
#' @keywords internal
#' @param input A vector of integers n model runs long, where the integers refer to soil class
#' occurrence probability.
#' @param n_classes Integer; the total number of soil classes available to predict.
#' @return An atomic vector of probabilities, or a vector of NA values of the same length. Ties are
#' shuffled at random.
#' e.g. for 20 model runs, e.g. 0 0.25 0.4 0 becomes [1] 0.4 0.25 0 0.
#' @note This is a helper function and not widely applicable. It is expected to be used in concert
#' with \code{\link[raster:calc]{calc}}, applied to a stack of tabulated model realisations produced
#' by \code{\link[dsmartr:collate]{collate}}.
#' @examples \dontrun{
...
#'   \item{\code{tallied_predictions}: RasterStack containing per-pixel tallies of soil
#'   class occurrence.}
#'   \item{\code{tallied_probabilities}: RasterStack containing per-pixel probabilities of
