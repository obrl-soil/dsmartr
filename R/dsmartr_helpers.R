...
#' @keywords internal
#' @param input An object of class sfc_POLYGON or sfc_MULTIPOLYGON, see
#' \code{\link{dsmartr_prepare}}.
...
#' library(dplyr)
#' library(tibble)
#' ex <- tribble( ~ID, ~CLASS_1, ~CLASS_2, ~PERC_1, ~PERC_2,
#'                  1,      'A',      'B',      60,      40)
#' n_things(ex, 'PERC')
...
#' @param src_map An sfc_POLYGON, sfc_MULTIPOLYGON, or SpatialPolygonsDataFrame object.
...
#' @examples
#' load("dst_sf_polygons")
#' checked_map <- dsmartr_check_polygons(src_map = dst_sf_polygons, id_field = 'NUID')
#' @export
