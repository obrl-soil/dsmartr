...
#'
#' Returns the set of unique soil classes that occur in an input soil map, and,
#' optionally, a set of known points.
#' @keywords internal
#' @param soilmap Data Frame; Returned by \code{\link{prep_polygons}}.
#' @param soilpoints Data Frame; Returned by \code{\link{prep_points}};
#'   optional.
#' @param col_stub String; stub of attribute column names holding soil class
#'   data
#' @return A factor holding the set of soil classes occurring in both datasets,
#'   sorted alphabetically. Internal to \code{\link{iterate}}.
#' @note It is up to the user to make sure the classification schema in both
#'   datasets matches/is compatible.
#' @examples \dontrun{
...
#' @param cellcol String; name of attriute holding list of raster cells to
#'   sample from
#' @param t_factor Integer; dirichlet dampener, supplied by parent function
#' @return A data frame containing two columns: weighted random allocation of
#'   soil classes, and cell numbers.
#' @examples \dontrun{
#' # run prep_polygons() and prep_points() examples, then:
...
#' @param t_factor Integer; dirichlet distribution modifier. Where map units
...
#' @param write_files String; choose 'native' to write outputs in GeoTIFF, GPKG,
#'   or csv as appropriate, or 'rds' for outputs in RDS format. Note that map
#'   rasters are always written as GeoTIFF.
#' @param write_samples Logical; whether to retain the covariate samples taken
#'   during each iteration. If \code{TRUE}, each set of samples is written to
#'   disk as GPKG or RDS.
#' @param resume_from Integer; If this function is interrupted, it can be
...
#' @return RasterStack; \code{n_iterations} layers. As a side effect, for each
