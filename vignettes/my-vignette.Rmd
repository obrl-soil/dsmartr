...
### 1. Prepare data for repeated sampling

Extract soils mapping data and format its attributes as for the demonstration dataset 'heronvale_soilmap'. Data can be stored on disk in any GDAL-readable format that R can turn into an sf or sp polygon object. Presence of multipolygons won't cause the model to fail, but the sample number chosen for a multipolygon will be spread across all subgeometries.

Assemble a set of raster covariates. These should all have the same extent, cell size, and CRS, and  underlie the input mapping. The covariates can have no-data areas, but if a polygon is 100% over a no-data area it will not be sampled. The covariates can have a larger extent than the input mapping, but bear in mind that the output model will likely become less reliable further away from the input data. Covariates should have a much smaller cell size than the input mapping polygons - anything larger than about 1/1000 the median polygon size will probably not produce useful outputs.

Optionally, assemble a set of points where the soil is known (described sites). A location and soil class that fits the mapping schema is all that is required. Data can be stored as spatial points or as a table with coordinates.

Use `dsmartr_prep_polygons()` on the input mapping data. This function takes the input mapping and adds four new fields - polygon area, number of soil types within each map unit, number of sample points desired, and a list-column containing all the raster cells whose centers intersect each polygon. At this stage, the polygon geometry is no longer required and is dropped. 
...
load(system.file('data/heronvale_soilmap.rda',     package='dsmartr'))
load(system.file('data/heronvale_covariates.rda',  package='dsmartr'))
load(system.file('data/heronvale_known_sites.rda', package='dsmartr'))
